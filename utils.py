...
def log(obj, filename='log_dice.txt'):
...
def make_optimizer(param_list, optimizer_spec, load_sd=False):
    ...
    # print(optimizer_spec['args']['lr'])
    # print(optimizer_spec['args'])
    if load_sd:
        optimizer.load_state_dict(optimizer_spec['sd'])
    return optimizer
#
# def make_optimizer(param_list, weight, optimizer_spec, load_sd=False):
#     print(weight)
#     optimizer = torch.optim.AdamW(
#         # 放入可训练参数
#         params=[
#             {'params': param_list, 'lr': optimizer_spec['args']['lr']},
#             {'params': weight, 'lr': 0.001, 'weight_decay': 0}
#         ],
#     )
#     if load_sd:
#         optimizer.load_state_dict(optimizer_spec['sd'])
...
def f1_calc(pred, true):
    ...
    f2_score = 0
    return precision, recall, f1, f2_score
...
def calc_f1(y_pred,y_true):
    ...
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        f1, auc, precision_sum, recall_sum, iou = 0, 0, 0, 0, 0

        p_sum, r_sum, f_sum, f2_sum = 0, 0, 0, 0
        ...
        for i in range(batchsize):
            # y_true_i = y_true[i]
            # 容忍误差 3
            y_true_i = np.squeeze(y_true[i])
            kernel = np.ones((3, 3))
            y_true_i = binary_dilation(y_true_i, structure=kernel)
            y_true_i = y_true_i.astype(np.float32)
            # print(y_true.shape)
            # print(np.unique(y_true))


            true = y_true_i.flatten()
            true = true.astype(np.int_)
            pred = y_pred[i].flatten()

            p_1, r_1, f_1, f_2 = f1_calc(pred, true)
            p_sum += p_1
            r_sum += r_1
            f_sum +=f_1
            ...
            # IoU
            # pred_iou = np.where(pred >= 0.5, 1, 0)
            # intersection = np.sum(true * pred_iou)
            # union = np.sum(np.logical_or(true, pred_iou))
            ...
            # dice系数
            intersection = 2 * (true * pred).sum()
            union = true.sum() + pred.sum()
            dice = (intersection + 1e-6) / (union + 1e-6)

    return f_sum/batchsize, dice/batchsize, iou/batchsize, p_sum/batchsize, r_sum/batchsize
